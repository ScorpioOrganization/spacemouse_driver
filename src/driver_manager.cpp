...
std::shared_ptr<Driver> DriverManager::create_driver(const std::vector<Model>& model_list) {
  ...
  for (const auto& model : model_list_cpy) {
    if (!DeviceRegistry::is_supported(model)) {
      _context->logger->error("Unsupported device model specified: " + std::string(magic_enum::enum_name(model)));
      return nullptr;
    }
  }
...
std::shared_ptr<Driver> DriverManager::make_driver(const std::shared_ptr<ConnectionMethod>& conn_method) {
